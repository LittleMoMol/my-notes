...
```
## 2-4 可视化
- 下图表示真实函数、5 次插值函数、10 次插值函数、15 次插值函数之间的关系（蓝色为原始函数，黄色为 5 次牛顿插值函数，绿色为 10 次牛顿插值函数，红色为 15 次牛顿插值函数）
	![[T1.png|500]]
- 从上面图片可以看出“龙格现象”：
	- 5 次插值多项式拟合程度较差
	- 10 次插值多项式在 $x=0$ 附近拟合程度较好，但在 $x$ 绝对值较大时呈现明显的病态，误差很大
	- 15 次插值多项式在 $x=0$ 附近拟合程度较好，但在 $x$ 绝对值较大时呈现明显的病态，误差很大
...
```
## 3-4 可视化
- 下图表示真实函数、分段二次插值函数 $N_{2,20}$ 的关系（蓝色为原始函数，黄色为 $N_{2,20}$）
	![[T2-1.png|450]]
- 从上图可以看出：分段二次插值与真实值接近，误差分布也比较均匀，远远好于等距节点下的高次牛顿插值函数
...
- 函数 `diff_vector(xi, yi, dy)`：负责通过 $d_{j}=6f[x_{j-1},x_{j},x_{j+1}]$ 和 $\left\{\begin{array}{l} 2M_{0}+M_{1}=\dfrac{6}{h_{1}} \left( \dfrac{f_{1}-f_{0}}{h_{1}} - f_{0}^{\prime} \right):=d_{0} \\ M_{n-1}+2M_{n} = \dfrac{6}{h_{n}}\left(f_{n}^{\prime}-\dfrac{f_{n}-f_{n-1}}{h_{n}}\right):=d_{n} \end{array}\right.$ 计算三弯矩方程右端常数项 $d$ 
	- 这里 $h_1 = x_1 - x_0,\ h_n = x_n - x_{n-1}$ 
- 函数 `init_M(hi, d)` 
	负责通过 $\lambda_{j}=\dfrac{h_{j}}{h_{j-1}+h_{j}},\ \mu_{j}=1-\lambda_{j},\ j=1,2,\cdots,n-1$ 和
	$$
	\left[\begin{array}{cccc}
	2 & 1 & & \\
	\mu_{1} & 2 & \lambda_{1} & \\
	...
	$$
	求解三弯矩方程的解j
	注意：因为要传入 `hi` 数组，而在计算机中下标从 $0$ 开始，故这里 $h_i$ 的定义改为 $h_i = x_{i+1} - x_i$ 
- 函数 `cubic_spline(xi, yi, dy, x)` 
	负责通过
	$$
	\begin{aligned} 
	s_{\text{I}}(x)=&\dfrac{1}{6h_{i-1}}[(x_{i}-x)^{3}M_{i-1}+(x-x_{i-1})^{3}M_{i}] + \dfrac{1}{h_{i-1}} [(x_{i}-x)f_{i-1} + (x-x_{i-1})f_{i}] \\ 
	&- \dfrac{h_{i-1}}{6}[(x_{i}-x)M_{i-1}+(x-x_{i-1})M_{i}],\quad x\in e_{i},\ i=1, 2, \cdots,  n
	\end{aligned}
	$$
	计算三次样条插值函数在点 $x$ 处的值
## 4-3 相关函数的流程图、源代码
...
def diff_vector(xi, yi, dy):
  ...
  d.append((dy[-1] - (yi[n - 1] - yi[n - 2]) / (xi[n - 1] - xi[n - 2])) * 6 / (xi[n - 1] - xi[n - 2]))
  return np.array(d)
```
### 4-3-2 `init_M(hi, d)` 
该函数的流程图如下：
...
def init_M(hi, d):
  '''
  输入相关已知量，返回三弯矩方程的解
  :param hi: list-like, 插值区间长度, hi[i] = xi[i+1] - xi[i]
  :param d: list-like, 三弯矩方程右端常数项
  :return: list-like, 三弯矩方程的解
  '''
  n = len(hi) + 1
...
def cubic_spline(xi, yi, dy, x):
  '''
  构造由 xi, yi, dy 产生的三次样条插值函数，并返回该函数在 x 点处的函数值
  :param xi: list-like, 插值节点的横坐标
  :param yi: list-like, 插值节点的函数值
  :param dy: list-like, 该 list-like 长度为 2, 用来存储 x0 和 xn 处的导数值 (I 型边界条件)
  :param x: float, 自变量
  :return: float, 三次样条插值函数值
  '''
  ...
  for i in range(n):
    if xi[i - 1] < x and x < xi[i]:
      res = ((xi[i] - x) ** 3 * M[i-1] + (x - xi[i-1]) ** 3 * M[i]) / (6 * hi[i-1])
      res += ((xi[i] - x) * yi[i-1] + (x - xi[i-1]) * yi[i]) / hi[i - 1]
      res -= hi[i - 1] / 6 * ((xi[i] - x) * M[i-1] + (x - xi[i-1]) * M[i])
      return res
```
## 4-4 可视化
- 下图表示真实函数、三次样条插值函数 $S_{10}$、三次样条插值函数 $S_{20}$ 的关系（蓝色为原始函数，黄色为 $S_{10}$，红色为 $S_{20}$）
	![[T2-2.png|450]]
- 从上图可以看出：
	- $S_{10}$ 误差较大的原因是因为节点个数不够，当节点个数增加到 $20$ 个的时候，$S_{20}$ 是和原函数最为接近的！
	- 三次样条插值 $S_{20}$ 的结果与原函数的曲线几乎完全重合，和牛顿插值、分段二次插值相比，三次样条插值插值效果最好！
...
- 由第 2 章知，函数 $f(x)$ 关于 $[-1, 1]$ 上彼此互异的节点 $x_{0}, x_{1}, \cdots, x_{n}$ 的 $n$ 次插值多项式 $p_{n}(x)$，有余项估计式 $\left\| f - p_{n} \right\| \leqslant \dfrac{\left\| f^{(n+1)} \right\|}{(n+1)!} \max\limits_{|x| \leqslant 1} \left| \left( x - x_{0} \right) \left( x - x_{1} \right) \cdots \left( x - x_{n} \right) \right|$ 
...
## 5-2 算法实现逻辑
- 函数 `get_chebyshev_point`：负责通过 $x_{k} = \dfrac{a+b}{2} + \dfrac{b-a}{2} \cos \dfrac{(2k+1)\pi}{2(n+1)}, \ k = 0, 1, 2, \cdots, n$ 生成 `[a,b]` 上的 $n+1$ 个插值节点
- 再通过使用 `Newton(xi, yi)` 进行牛顿插值
## 5-3 相关函数的流程图、源代码
### 5-3-1 `get_chebyshev_point` 
该函数十分简单，故不画流程图
该函数的源代码在文件 `function.py` 中，内容如下：
```python
def get_chebyshev_point(a, b, n):
  '''
  :param a: float, 区间左端点
  :param b: float, 区间右端点
  :param n: int, 生成的节点个数
  :return: list-like, 生成的 n 个节点的横坐标
  '''
  xi = [((a + b)/2 + (b - a) / 2 * cos((2 * k + 1) * pi / (2 * (n + 1)))) for k in range(0, n+1)]
  return np.array(xi)
```
## 5-4 可视化
- 下图表示真实函数、切比雪夫多项式零点为插值节点的 $N_{10}$、切比雪夫多项式零点为插值节点的 $N_{20}$、切比雪夫多项式零点为插值节点的 $N_{30}$ 的关系（蓝色为原始函数，黄色为 $N_{10}$，绿色为 $N_{20}$、红色为 $N_{30}$）
	![[T3 1.png|450]]
- 从上图可以看出：
	- $N_{10}, N_{20}, N_{30}$ 都不会出现龙格现象，并且随着插值点数的增多，插值多项式的逼近效果也越来越好
	- 与等距节点的 Newton 多项式相比，可以看出，插值节点的选取对插值多项式的质量有很大的影响！如何选取正确的插值节点至关重要。

# 6 插值的应用：图像的放大与旋转
图片在计算机中以数字矩阵的形式存储。无论是放大还是旋转操作，其核心问题都是如何为原始像素网格中没有对应位置的“新像素点”计算其颜色 (亮度) 值。
## 6-1 问题描述
- 一张分辨率为 $W \times H$ 的图片，可以看作定义在二维整数网格点集 $\{(i, j) | i=0,1,\cdots , W-1;  j = 0,1,\cdots,H-1\}$ 上的离散函数 $I(i, j)$，其函数值为该点的像素值 (如 RGB 分量或灰度值)
* **图片放大** 
	将图片放大 $k$ 倍，意味着目标图片的分辨率变为 $(kW) \times (kH)$ 
	我们需要为所有新的像素位置 $(x', y')$ (通常是亚像素位置，即坐标可能不是整数) 计算像素值 $I'(x', y')$ 
* **图片旋转** 
	图片绕某点 (通常为中心) 旋转角度 $\theta$ 后，原图中像素点 $(i, j)$ 会映射到新图中的位置 $(x', y')$，反之亦然。
	这个映射关系由旋转变换矩阵给出：$\begin{bmatrix} x' \\ y' \end{bmatrix} = \begin{bmatrix} \cos\theta & -\sin\theta \\ \sin\theta & \cos\theta \end{bmatrix} \begin{bmatrix} i \\ j \end{bmatrix}$ 
	然而，计算出的 $(x', y')$ 通常不是整数坐标。
	为了填充目标图像的整数网格，我们通常采用**反向映射**：对于目标图像的每个整数坐标 $(u, v)$，通过逆变换计算其在原图中的对应位置 $(x, y)$：$\begin{bmatrix} x \\ y \end{bmatrix} = \begin{bmatrix} \cos\theta & \sin\theta \\ -\sin\theta & \cos\theta \end{bmatrix} \begin{bmatrix} u \\ v \end{bmatrix}$ 
	这个 $(x, y)$ 也通常是亚像素位置。
	我们的任务就是根据原图在周围整数像素点的值，来估算 $I(x, y)$ 的值。
- 接下来，我们以图片放大为例，进行核心步骤的展开
## 6-2 核心步骤
- *符号规定* 
	假设原图像放大 $scale$ 倍得到目标图像
	令函数 $I(i,j)$ 为 $\mathbb R^2 \to \mathbb R^3$ 的一个函数，对于给定点像素点 $(i,j)$，返回其 $RGB$ 值 $(R, G, B)$ 
	对于目标函数上的一个像素点 $(dst_x, dst_y)$，我们可以令 $(src_x, src_y) := \left( \dfrac{dst_x}{scale}, \dfrac{dst_y}{scale} \right)$ 为原图像与之相应的点。显然该点几乎不可能是整数点，而是亚像素
	我们令 $i = \lfloor src_x \rfloor,\ j = \lfloor src_y \rfloor,\ u = src_x - i,\ y = src_y - j$，可知 $(i,j),\ (i+1, j),\ (i, j+1),\ (i+1, j+1)$ 包围着 $(src_x, src_y)$ 
- 接下来看如何求解 $I(src_x, src_y)$ 
	*第一步* 
	我们可以先用 $(i, j),\ (i+1, j)$ 两点做 $i$ 方向的一维一次线性插值，得到 $I(x, j) = \dfrac{x-(i+1)}{i-(i+1)} I(i, j) + \dfrac{x-i}{i+1-i} I(i+1,j)$，化简得 $I(x, j) = (x-i) I(i+1, j) + (i+1-x) I(i,j)$ 
	令 $x = src_x$ 代入 $I(x,j)$ 得到 $I(src_x, j) = uI(i+1, j) + (1-u)I(i,j)$ 
	*第二步* 
	再用 $(i,j+1), (i+1,j+1)$ 两点做 $i$ 方向的一维一次线性插值，得到 $I(x, j+1) = \dfrac{x - (i+1)}{i-(i+1)} I(i,j+1) + \dfrac{x-i}{i+1-i} I(i+1, j+1)$，化简得 $I(x,j+1) = (x-i)I(i+1, j+1) + (i+1-x)I(i, j+1)$ 
	令 $x=src_x$ 代入 $I(x,j+1)$ 得到 $I(src_x, j+1) = uI(i+1, j+1) + (1-u)I(i, j+1)$ 
	*第三步* 
	最后用 $(src_x, j), (src_x, j+1)$ 两点做 $j$ 方向的一维一次线性插值，得到
	$I(src_x, y) = \dfrac{y-(j+1)}{j-(j+1)} I(src_x, j) + \dfrac{y-j}{j+1-j} I(src_x, j+1)$，化简得 $I(src_x, y) = (y-j) I(src_x, j+1) + (j+1-y) I(src_x, j)$ 
	令 $y=src_y$ 代入 $I(src_x, y)$ 得到 $I(src_x, src_y) = vI(src_x, j+1) + (1-v)I(src_x, j)$ 
	综上可得 $I(src_x, src_y) = v(uI(i+1, j+1) + (1-u)I(i, j+1)) + (1-v)I(uI(i+1,j) + (1-u)I(i,j))$ 
- 我们直接令 $I(dst_x, dst_y) = I(src_x, src_y)$ 即可得到放大后的图像在 $(dst_x, dst_y)$ 点处的 $RGB$ 值
- *注意*：放大和旋转只是矩阵系数不同，但它们进行像素点求 $RGB$ 的过程都是利用相同的插值原理得到的，故这里只讨论图片放大的应用

# 7 附录
本次实验报告在文件夹 `experiment_report_1` 下共有 `function.py`、`T1.py`、`T2.py`、`T3.py` 4 个 python 文件
...
plt.plot(x, func(x), label="True Value")

# 5 次 Newton 插值
xi = np.linspace(-10, 10, 6)
yi = func(xi)
N5 = f.Newton(xi, yi)
plt.plot(x, N5(x), label="Newton-5")

# 10 次 Newton 插值
xi = np.linspace(-10, 10, 11)
yi = func(xi)
N10 = f.Newton(xi, yi)
plt.plot(x, N10(x), label="Newton-10")

# 15 次 Newton 插值
xi = np.linspace(-10, 10, 16)
yi = func(xi)
N15 = f.Newton(xi, yi)
plt.plot(x, N15(x), label="Newton-15")
...
yi = func(xi)
plt.plot(x, [f.sp2(xi, yi, _) for _ in x], label="Piecewise Quadratic")
plt.legend(loc="upper left")
plt.savefig("T2-1.png", dpi=500)
...
xi = np.linspace(-10, 10, 11)
yi = func(xi)
dfunc = lambda x: -2 * x / (1 + x ** 2) ** 2
dy = [dfunc(xi[0]), dfunc(xi[-1])]
plt.plot(x, [f.cubic_spline(xi, yi, dy, _) for _ in x], "--", label = "CubicSpline-10")
...
dfunc = lambda x: -2 * x / (1 + x ** 2) ** 2
dy = [dfunc(xi[0]), dfunc(xi[-1])]
plt.plot(x, [f.cubic_spline(xi, yi, dy, _) for _ in x], "--", color="r", label = "CubicSpline-20")

plt.legend(loc="upper left")
plt.savefig("T2-2.png", dpi=500)
```
...
# 以切比雪夫多项式零点为插值节点的 10 次 Newton 多项式
xi = f.get_chebyshev_point(-10, 10, 11)
yi = func(xi)
N10 = f.Newton(xi, yi)
plt.plot(x, N10(x), label="Newton-10")

# 以切比雪夫多项式零点为插值节点的 20 次 Newton 多项式
xi = f.get_chebyshev_point(-10, 10, 20)
yi = func(xi)
N20 = f.Newton(xi, yi)
plt.plot(x, N20(x), label="Newton-20")

# 以切比雪夫多项式零点为插值节点的 30 次 Newton 多项式
xi = f.get_chebyshev_point(-10, 10, 30)
yi = func(xi)
N30 = f.Newton(xi, yi)
plt.plot(x, N30(x), label="Newton-30")
