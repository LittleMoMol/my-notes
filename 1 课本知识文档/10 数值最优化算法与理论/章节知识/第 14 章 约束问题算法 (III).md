...
- *算法 14.3 注释* 
	1. 上面算法中的步1给出的终止准则等价于 $x^{(k)}$ 是问题 $(q1)$ 的 K-K-T 点
	2. 算法中的*第 5 步*包括试探步可接受的判断和信赖域半径的调节，其中 $r_k$ 描述了近似模型与原问题一致性程度。当 $r_k < \eta_1$，信赖域 SQP 子问题的解被拒绝，需缩小信赖半径重新解子问题，否则接受子问题的解，并根据 $r_k$ 的值确定保持或放大下次迭代计算的信赖域半径
- *假设条件* 
